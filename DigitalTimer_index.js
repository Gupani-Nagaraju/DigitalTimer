...
class DigitalTimer extends Component {
  ...
  renderTimerLimitController = () => {
    ...
    const isButtonDisabelled = timerElapsedInSeconds > 0
      ...
      <div>
        ...
        <div>
          <button
            ...
            disabled={isButtonDisabelled}
            type="button"
          >
            -
          </button>
          <div>
            <p>{timerLimitsInMinutes}</p>
          </div>
          <button
            type="button"
  ...
  incrementTimerLimitInSeconds = () => {
    const {timerElapsedInSeconds, timerLimitsInMinutes} = this.state
  ...
  onStartOrPauseTimer = () => {
    const {isTimerRunning, timerElapsedInSeconds, timerLimitsInMinutes} =
      this.state
    const isTimerCompleted = timerElapsedInSeconds === timerLimitsInMinutes * 60
    ...
    if (isTimerRunning) {
      ...
      this.intervalId = setInterval(this.incrementTimerLimitInSeconds, 1000)
    }
    this.setState(prevState => ({
